...
<div class="flex flex-col gap-4 p-4 shadow bg-base-200 rounded-2xl">
    ...
    <div class="flex items-center gap-4 mt-4">
        <a href="{{ $isAdmin ? route('admin.providers.recruitments.index', $rec->rc_u_id) : route('provider.recruitments.index') }}" class="flex items-center justify-center w-9 h-9 border border-gray-400 rounded-2xl bg-base-100 hover:bg-gray-200 transition" title="กลับ">
            <i class="fa-solid fa-arrow-left text-gray-600"></i>
        </a>
        <div class="min-w-0">
            <h1 class="text-2xl font-bold leading-tight md:text-3xl text-base-content break-words">แก้ไขประกาศงาน</h1>
            <p class="mt-1 text-sm text-gray-500">{{ $rec->rc_title }}</p>
        </div>
    </div>
    ...
    <form id="recruitment-form" method="POST" novalidate
        ...
        @php
            $selectedType = old('rc_type', $rec->type_values[0] ?? '');
            $selectedWorkMode = old('rc_work_mode', $rec->work_mode_values[0] ?? '');
            $existingSkills = collect(old('skills', $rec->skills->map(fn ($skill) => [
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <textarea name="rc_description" rows="4"
                            ...
                            placeholder="ระบุรายละเอียดงาน เช่น หน้าที่ความรับผิดชอบ ลักษณะงาน...">{{ old('rc_description', $rec->rc_description) }}</textarea>
                    ...
                    <div id="location-text-field" class="md:col-span-2">
                        ...
                        <select name="rc_location_text" id="rc_location_text" class="w-full select select-bordered focus:select-primary border border-base-300 pl-2">
                            ...
                            @foreach(config('th_provinces', []) as $province)
                                <option value="{{ $province }}" @selected(old('rc_location_text', $rec->rc_location_text) === $province)>{{ $province }}</option>
                            @endforeach
                        </select>
            ...
            <div class="bg-base-200 rounded-2xl px-5 py-4">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-2">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <input type="hidden" name="rc_salary" id="rc_salary" value="{{ old('rc_salary', $rec->rc_salary) }}">
                        ...
                        <div class="skill-row grid items-center gap-2 p-3 bg-base-100 rounded-xl border border-base-300"
                            ...
                            <div class="relative min-w-0 overflow-hidden">
                                ...
                                <div class="select select-bordered select-sm w-full flex items-center pointer-events-none overflow-hidden">
                                    @php $groupName = $skillGroups->firstWhere('id', $selectedGroupId)?->name ?? '-- กลุ่มทักษะ --'; @endphp
                            ...
                            @php
                                ...
                                $selectedSkillId = $existingSkill['skill_id'] ?? null;
                            @endphp
                            <div class="relative min-w-0 overflow-hidden">
                                <select name="skills[{{ $i }}][skill_id]"
                                    ...
                                    @if ($selectedGroup)
                                        @foreach ($selectedGroup->skills->sortBy(fn ($s) => mb_strtolower($s->name)) as $skill)
                                            <option value="{{ $skill->id }}" @selected($selectedSkillId == $skill->id)>{{ $skill->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                                <div class="select select-bordered select-sm w-full flex items-center pointer-events-none overflow-hidden">
                                    @php $skillName = optional($selectedGroup?->skills->firstWhere('id', (int) $selectedSkillId))->name ?? '-- ทักษะ --'; @endphp
                            ...
                            <select name="skills[{{ $i }}][proficiency_level]"
                                ...
                                @foreach (['beginner' => 'เริ่มต้น', 'intermediate' => 'ปานกลาง', 'advanced' => 'ขั้นสูง', 'expert' => 'ผู้เชี่ยวชาญ'] as $level => $label)
                                    <option value="{{ $level }}" @selected(($existingSkill['proficiency_level'] ?? 'beginner') === $level)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <button type="button"
                                class="remove-skill btn btn-xs shrink-0"
        ...
        <div class="bg-base-200 rounded-2xl px-5 py-4 mt-3">
            ...
            <div class="mt-3">
                ...
                <textarea name="rc_requirements" rows="4"
                    class="w-full textarea textarea-bordered focus:textarea-primary border border-base-300 pl-2"
                    placeholder="ระบุข้อมูลเพิ่มเติมเกี่ยวกับคุณสมบัติ (ถ้ามี)">{{ old('rc_requirements', $rec->rc_requirements) }}</textarea>
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="date" name="rc_expire_at" id="rc_expire_at"
                        class="w-full input input-bordered focus:input-primary border border-base-300 pl-2"
                        value="{{ old('rc_expire_at', optional($rec->rc_expire_at)->format('Y-m-d')) }}" min="{{ $tomorrow }}">
                    ...
                    <div class="mt-2">
                        <label class="inline-flex items-center gap-3 cursor-pointer px-3 py-2 rounded-lg border border-base-300 bg-base-100 transition hover:border-blue-300 hover:bg-blue-50">
                            <input type="checkbox" id="expire_no_limit" name="expire_no_limit" value="1"
                                class="checkbox checkbox-sm border-2 border-slate-500 bg-white checked:bg-blue-600 checked:border-blue-600"
                                @checked(old('expire_no_limit', !$rec->rc_expire_at ? '1' : '0') === '1')>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const validateForm = () => {
        ...
        document.getElementById('recruitment-form').querySelectorAll('[required]').forEach(field => {
            if (field.disabled) {
                return;
            }
            ...
            if (!field.value.trim()) {
                ...
                field.parentElement.querySelector('.native-err')?.remove();
            }
        });
    ...
    document.getElementById('recruitment-form').querySelectorAll('[required]').forEach(field => {
        field.addEventListener('input', () => field.parentElement.querySelector('.native-err')?.remove());
        field.addEventListener('change', () => field.parentElement.querySelector('.native-err')?.remove());
    });
