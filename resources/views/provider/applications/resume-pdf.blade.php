...
<head>
    <meta charset="utf-8">
    <title>Resume - {{ $application->resume->first_name }} {{ $application->resume->last_name }}</title>
    <style>
...
<body>
    <h1>{{ trim(($application->resume->first_name ?? '').' '.($application->resume->middle_name ?? '').' '.($application->resume->last_name ?? '')) }}</h1>
    <p class="muted">สมัครตำแหน่ง: {{ $application->recruitment->rc_title }}</p>

    <table class="grid">
        <tr>
            <td width="50%"><strong>Email:</strong> {{ $application->resume->email ?? $application->jobber->email ?? '-' }}</td>
            <td width="50%"><strong>Phone:</strong> {{ $application->resume->phone ?? '-' }}</td>
        </tr>
        <tr>
            <td><strong>วันเกิด:</strong> {{ $application->resume->birth_date ? \Carbon\Carbon::parse($application->resume->birth_date)->format('d/m/Y') : '-' }}</td>
            <td><strong>เพศ:</strong> {{ ['male' => 'ชาย', 'female' => 'หญิง', 'other' => 'อื่นๆ'][$application->resume->gender] ?? '-' }}</td>
        </tr>
    </table>
    ...
    @if($application->resume->certificates->count())
        ...
        @foreach($application->resume->certificates as $cert)
            <div class="item">
                ...
                @if($cert->issued_by)
                    <p class="muted">{{ $cert->issued_by }} {{ $cert->issued_year ? ' | '.$cert->issued_year : '' }}</p>
                @endif
            </div>
        @endforeach
    @endif
